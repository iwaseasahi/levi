...
if [[ "$backup_tier" != "operational" && "$backup_tier" != "weekly" ]]; then
  echo "Usage: production-backup.sh operational|weekly" >&2
  exit 2
fi
...
prepare_directory "${backup_root}/operational"
...
find "${backup_root}/operational" -type f -mmin +2880 -delete
find "${backup_root}/weekly" -type f -mmin +43200 -delete
if [[ -d "${backup_root}/hourly" ]]; then
