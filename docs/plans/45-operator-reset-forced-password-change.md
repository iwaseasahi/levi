...
- 2026-08-21 15:54 JST — PR #68 exposed slow authentication navigation. Removed
  redundant refreshes from login, password-change completion, and logout. The
  CI trace then confirmed successful sign-in took about four seconds for scrypt
  on the two-core runner, exhausting the five-second URL assertion during the
  following server navigation. Authentication-transition assertions now use a
  scoped 20-second budget; ordinary UI assertions retain their default limit.
