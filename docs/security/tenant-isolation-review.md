...
| Preview and edit route                                 | Preview validates an unsaved local document, sends no request and changes no audience. Editor is page-gated; foreign detail/edit never receive a Slide. `slide-editor.component.test.tsx`, `slide-editor.spec.ts`, `slide-security.spec.ts`.                                                                                                                                                        |
...
| Cross-window trust                                     | Strict v2 schema, origin + exact Window, content kind/generation, challenge/document instance and monotonic sequences; no Slide content in messages. `transport.test.ts`, `transport.component.test.tsx`, `projection-connection.spec.ts`.                                                                                                                                                          |
| HTML/log/URL/storage disclosure                        | Rich input is reduced to validated versioned text runs and rendered as React text spans; no raw HTML. Paste discards markup/files. Audience URL is ID/page plus nonce only. APIs no-store; logger allowlist and recursive sensitive-key redaction covers title/body/document/query. `slide-tiptap.test.ts`, `slide-projection.spec.ts`, `logger.test.ts`; no Slide localStorage or content logging. |
