...
| All/recent/body search and cursor reuse                | Parameterized scoped SQL; body-only literal matching; bounded metadata-only results; forged cursor fields rejected. `slide-search.test.ts`, `slide-search.spec.ts`, `slide-security.spec.ts`.                                                                                                                                |
...
separately covered by #389; large substring query cost remains #397. Final
cross-flow acceptance and parity evidence are owned by #390.
