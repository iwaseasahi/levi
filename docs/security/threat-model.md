...
| Setup/reset token disclosure          | Invitation and password-recovery email links                        | Generate server-side, store only Better Auth verification state, expire after 24 hours, make links single-use, never log tokens, and revoke sessions after successful reset                                                            | Recipient mailbox security and Gmail delivery remain external dependencies                    |
