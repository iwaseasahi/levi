...
| ------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Identity spoofing/session theft       | Login, reset, DB session, cookies, controller/audience channel | Better Auth `scrypt`, hashed reset identifiers, host-only secure cookie, DB revocation, seven-day expiry, no cookie cache, generic errors, auth tests               | Session lookup token remains Restricted at rest; harden DB/backups and patch latest Better Auth |
| Privilege escalation/IDOR             | Platform routes and every church-owned lookup                  | Better Auth proves identity only; Levi derives actor/membership/tenant in the DAL and denies guessed/cross-tenant IDs at use-case and repository layers             | Issues #43, #44, and #55 prove allowed and denied paths                                         |
...
| Denial of service/resource exhaustion | Login/reset, search, rendering, DB pool                        | PostgreSQL-backed endpoint-specific auth limits, bounded inputs/timeouts, health/readiness separation, DB pool, query/index review                                  | Trusted proxy topology and capacity targets await deployment                                    |
| Account/email abuse                   | Login, reset request, transactional email                      | Generic responses, asynchronous send, one-hour single-use reset, quota-aware provider port, no external email in local/CI                                           | Resend account/domain/credential and production limits require approval                         |
