...
The `/slides` UI now shows the all-list directly, with no recent/all switches or
body-search form. List component/E2E cover rows, title/author, pagination, stale
responses, empty/error/retry and 390px/1280px geometry. `slide-search.spec.ts`
retains recent/literal/no-match assertions through the authenticated API;
search domain/controller/integration coverage is unchanged. The lifecycle uses
the list to open saved content. Original search delivery evidence remains above.
