...
[`product/slide-contract.md`](product/slide-contract.md), ADRs 0015–0017.

## Acceptance map

| Behavior                                                                                          | Executable evidence                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Create → document-only unsaved preview → save → list → projection → edit/reopen → physical delete | `tests/e2e/slide-lifecycle.spec.ts` uses the UI across controller, editor and audience. Confirms preview does not save/open/project, including while another saved Slide is on screen.                                                                                                                                              |
| Validation, EOL, selected-range sizes, consecutive LF, Unicode and literal HTML                   | `slide.test.ts`, `text-document.test.ts`, `slide-tiptap.test.ts`, editor component/E2E, projection E2E. Preview/editor/audience share the relative-size fit model.                                                                                                                                                                  |
| Whole-body top/center/bottom placement and centered compatibility                                 | `slide.test.ts`, editor/projection components, CRUD/schema integration, and `slide-lifecycle.spec.ts` cover strict values, defaulting, distinct accessible controls, focus retention, unsaved preview, persistence/reload/detail/audience, responsive fit, and blank recovery. Image component/schema tests prove exclusion.        |
...
| List-row deletion beside favorite                                                                 | `slide-list.spec.ts` and list component tests verify right-side placement, title-bearing confirmation, cancel, serialized pending state, revision payload, retained failure and removal only after HTTP 204.                                                                                                                        |
| Text/image type label before each list title                                                      | `slide-list.spec.ts`, list component tests and list integration coverage verify normalized `contentType`, visible Japanese labels, accessible link names, protected-content omission and narrow/wide layout.                                                                                                                        |
| Concurrent edit and stale delete                                                                  | Lifecycle E2E has two real editor tabs: 409 preserves the unsaved title/document and focuses the error; the original saved revision/document remains. CRUD integration/API E2E covers stale delete and racing writers.                                                                                                              |
| Complete-document projection and acknowledged blank control                                       | `slide-projection.spec.ts`, lifecycle E2E, projection component/transport tests. Slide projection exposes no transient font controls; Scripture retains the shared transport font-scale responsibility.                                                                                                                             |
| Document-only API/domain contract                                                                 | `slide.test.ts`, Slide controller tests, `slide-crud.test.ts`, `slide-image.test.ts`, and `slide-api.spec.ts` prove legacy `body` input rejection, document-only text responses, image responses without `body: null`, strict persisted-document reads, and local flatten validation.                                               |
...
| Existing published workflows                                                                      | Entire scripture/search/bookmark/folder/auth/operator suite runs unchanged except the separately merged #400 synthetic Basic-auth fixture correction; `slide-list.spec.ts` also preserves the selected folder across Scripture/Slide bookmark navigation and reload.                                                                |
...
The audience displays document content, blank or generic recovery feedback only. Revalidation
