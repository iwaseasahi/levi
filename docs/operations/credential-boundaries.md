...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Local development/test    | Disposable loopback PostgreSQL values from `.env.example`; local Codex ChatGPT subscription login stored outside the repository | Repository-scoped Codex may use only the in-scope disposable database value       | Ignored `.env`; Codex login remains in the user-owned client configuration; never commit/log/copy into PR        |
...
The Better Auth secret is an application runtime secret, not a Codex
credential. Its creation, rotation, and production injection require an
