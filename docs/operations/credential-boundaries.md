...
The Better Auth secret and transactional-email credential are application
runtime secrets, not Codex or Claude Code credentials. Their creation, rotation,
provider configuration, and production injection require an exact human-approved
operation. Local and CI authentication tests use synthetic secrets and an
in-memory email adapter; they never receive a Resend or other delivery credential.
